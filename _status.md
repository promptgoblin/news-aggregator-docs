...
- **Replay results before shipping** (Aug 28–Sep 7 window, real clusters, summary-only): 9s 16 → 4 (Fable 5.1, GPT-6 Astra, the Pentagon ruling, the multi-provider outage — every one names a gate); exact agreement with editor adjustments 15/18 vs 0/18 for the old prompt; 8s landed ~4% of events, under the one-in-six target — tune on live data. Deployed 2026-09-07 06:47 UTC.
- **DeepMind feed re-ingested every run since Aug 13** (redirecting feed links defeated URL dedup; 825 duplicate rows, 239 junk updates, ~$0.3/run): dedup now matches the feed's original link; `scripts/repair_duplicate_articles.py` cleans up. `knowledge/gotcha_feed_redirect_dedup.md`. Index-scrape sources now bootstrap on first poll (record existing posts as seen, ingest none) so undated history never enters scoring.
- **Source expansion (31)**: Cognition + Devin blogs (the Windsurf → Devin Desktop rename of 2026-06-02 was never ingested), Cursor changelog, GitHub changelog/AI blog, Thinking Machines, Apple ML, Sakana, Together, Ollama, vLLM, Databricks, Midjourney, Sourcegraph, Lovable, VentureBeat AI, builder blogs (Simon Willison, Latent Space, Interconnects, Ahead of AI, One Useful Thing), and feedless blogs via the `lab_scrape` adapter (Anthropic Engineering, Cohere re-activated, ElevenLabs, Groq, Cerebras, Fireworks, Runway, World Labs, Physical Intelligence). Apply with `python -m ai_signal.seed` after deploy.
