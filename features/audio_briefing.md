...
- **Storyline context (Mike, 2026-07-09) — depends on `storylines.md`:** if the event belongs to a storyline, also pass that storyline's **context object** (`title` + `narrative_md`/"story so far" + `whats_new`). This lets the writer give a **situational take** ("the latest turn in the ongoing [X] saga…") instead of a robotic one-event summary. This is a primary reason storylines is built before the full podcast.
- **Update-awareness (Mike, 2026-07-09):** flag events that got a meaningful `EventUpdate` (new article/angle) since they were last covered on-air. Pass `update_count` + the recent `update_summary` list so the writer can say "[outlet] is now also reporting this, adding [angle]" — and so a previously-covered story with a real new development can earn a brief re-mention rather than being skipped as "already covered."
...
- **Situational takes from storyline context (when available):** if the event carries storyline context, use it for a *brief* situating clause — "the latest in the ongoing [X] story; last week [Y], now [Z]" or "[outlet] is now also reporting this, adding [angle]" for updates. This is what makes narration feel like a knowledgeable briefer, not a summarizer. Keep it to one clause — context, not a history lesson — and only when it genuinely adds. Facts still only from provided data (storyline narrative + event fields).
